      ...
      return <div className="po-page po-deposit-detail">
        ...
        {payout.originalExpectedCents === null ? <section className="po-pending-deposit"><Icon name="clock" /><h2>Awaiting processor batch</h2><p>This is normal for the current business day. It does not block the location’s operational close.</p></section> : <div className="po-settlement-layout">
          <section className="po-ledger-card">
            ...
            <div className="po-ledger-lines"><div><span>Captured payments</span>{payout.capturedCents === null ? <span>Not available</span> : <Money cents={payout.capturedCents} />}</div><div><span>Refunds</span>{payout.refundCents === null ? <span>Not available</span> : <Money cents={-payout.refundCents} signed />}</div><div><span>Processor fees</span>{payout.feeCents === null ? <span>Not available</span> : <Money cents={-payout.feeCents} signed />}</div>{adjustments.length > 0 && <div><span>Recorded adjustments</span><Money cents={adjustments.reduce((sum, item) => sum + item.amountCents, 0)} signed /></div>}<div className="po-ledger-total"><span>{adjustments.length ? 'Adjusted expected deposit' : 'Expected deposit'}</span>{payout.adjustedExpectedCents === null ? <span>Not available</span> : <Money cents={payout.adjustedExpectedCents} />}</div><div className="po-ledger-observed"><span>Observed bank deposit</span>{payout.observedBankCents === null ? <span>Not available</span> : <Money cents={payout.observedBankCents} />}</div><div className={payout.varianceCents === 0 ? 'po-ledger-variance po-ledger-variance--clear' : 'po-ledger-variance'}><span>Variance</span>{payout.varianceCents === null ? <span>Not available</span> : <Money cents={payout.varianceCents} />}</div></div>
