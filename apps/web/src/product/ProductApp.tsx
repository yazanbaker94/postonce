...
function Status({ value, compact = false }: { value: string; compact?: boolean }) {
  ...
  const tone = ['VERIFIED', 'POSTED · VERIFIED', 'READY', 'CLOSED', 'RECONCILED', 'CONNECTED', 'RESOLVED'].includes(normalized)
    ? 'verified'
    : ['OPEN', 'NEEDS_REVIEW', 'REVIEW'].includes(normalized)
      ? 'review'
      ...
      return (
        <div className="po-page">
          ...
          <div className="po-filterbar po-filterbar--payments">
            <label className="po-filter-search"><Icon name="search" /><span className="po-sr-only">Search payments</span><input value={query} onChange={(event) => update('q', event.target.value)} placeholder="Customer, record, amount, card, operation…" /></label>
            <label>Location<select value={location} onChange={(event) => update('location', event.target.value)}><option value="ALL">All locations</option>{state.rooftops.map((item) => <option key={item.id} value={item.code}>{item.name}</option>)}</select></label>
            <label>Department<select value={department} onChange={(event) => update('department', event.target.value)}><option value="ALL">All departments</option><option value="SERVICE">Service</option><option value="PARTS">Parts</option><option value="SALES">Sales</option></select></label>
            <label>Payment<select value={paymentState} onChange={(event) => update('paymentState', event.target.value)}><option value="ALL">All payment states</option>{[...new Set(state.payments.map((item) => item.paymentState))].sort().map((value) => <option key={value} value={value}>{sentenceCase(value)}</option>)}</select></label>
            <label>DMS<select value={dmsState} onChange={(event) => update('dmsState', event.target.value)}><option value="ALL">All DMS states</option>{[...new Set(state.payments.map((item) => item.dmsState))].sort().map((value) => <option key={value} value={value}>{dmsBusinessLabel(value)}</option>)}</select></label>
            <label>Settlement<select value={settlementState} onChange={(event) => update('settlementState', event.target.value)}><option value="ALL">All settlement states</option>{[...new Set(state.payments.map((item) => item.settlementState))].sort().map((value) => <option key={value} value={value}>{sentenceCase(value)}</option>)}</select></label>
          ...
          <div className="po-table-wrap">
            <table className="po-table">
              ...
              <tbody>{items.map((payment) => {
                const rooftop = state.rooftops.find((item) => item.id === payment.rooftopId)!;
                const record = state.dmsRecords.find((item) => item.id === payment.linkedRecordId);
                return <tr key={payment.id}><td><Link to={`/app/payments/${payment.id}`}><strong>{payment.id}</strong><small>{payment.methodType} •••• {payment.cardLast4} · {localTime(payment.receivedAt)}</small></Link></td><td><strong>{payment.customerLabel}</strong><small>{record?.recordNumber ?? 'Decision required'}</small></td><td>{rooftop.code}<small>{rooftop.name.replace('Northline ', '')}</small></td><td>{sentenceCase(payment.department)}</td><td><Money cents={payment.kind === 'REFUND' ? -payment.amountCents : payment.amountCents} signed={payment.kind === 'REFUND'} /></td><td><Status value={dmsBusinessLabel(payment.dmsState)} compact /></td><td><Status value={payment.settlementState} compact /></td><td><Link className="po-row-link" to={`/app/payments/${payment.id}`} aria-label={`Open ${payment.id}`}><Icon name="chevron" /></Link></td></tr>;
              })}</tbody>
            </table>
